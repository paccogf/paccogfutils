...
/*
#define CHECK_PREFIX(prefix) if(prefix) return 0;\
                             prefix=TRUE;\
                             finished=FALSE;\
                             opcode++;*/

#define CHECK_PREFIX(prefix) finished=FALSE;\
                             opcode++;
...
do 
{
    ...
    case 0x65:  // PREFIJO DE SEGMENTO GS:
                CHECK_PREFIX(bSegPrefix);
                break;

    case 0xF0:  // LOCK (PREFIJO)
                CHECK_PREFIX(bLockPrefix);
                break;

    case 0xF2:  // REPNE (PREFIJO)
                CHECK_PREFIX(bRepnePrefix);
                break;

    case 0xF3:  // REP/REPE (PREFIJO)
                CHECK_PREFIX(bRepePrefix);
                break;
                //////////////////////////////////////////////////////////////////
...
*/
    ...
    case 0xF7:  // IDIV EAX, REG
                if((*(opcode+1) & 0x38) == 0) opsize += b66Prefix? 2: 4; // TEST CON INMEDIATO DE 1 BYTE
                bModRegRM=TRUE;
                break;
...
if(bModRegRM)
{
    ...
    {
        if(((*opcode & 0x07) == SIB_BYTE) && (!b67Prefix)) // SIB BYTE PRESENTE
        {
            opsize++;
        }
        switch(*opcode >> 6) // NOS QUEDAMOS CON LOS 2 BYTES MAS ALTOS
        {
            ...
            case 2: opsize += b67Prefix? 2: 4; // DESPLAZAMIENTO 32 Ó 16 BITS
                    break;
